...
export class UsersService {
  ...
  async create(userDto:UserCreateDto) {
   const userExisted =  await this.userModel.findOne({ username: userDto.username }).exec()

   if(userExisted) throw new HttpException("User already exists", HttpStatus.BAD_REQUEST)
